module RedBlackTree_FSharp.RedBlackTree_Original
...
let insert tree x =
    let rec ins t =
        ...
        | Node(color, left, y, right) ->
            if x < y then
                (=<<) balance (apply (fun l -> Node(color, l, y, right)) (ins left))
            elif x > y then
                (=<<) balance (apply (fun r -> Node(color, left, y, r)) (ins right))
            else
                done' t
...
let rec delete tree x =
    let rec del t =
        ...
        | Node(color, left, y, right) ->
            if x < y then
                (=<<) delLeft (apply (fun l -> Node(color, l, y, right)) (del left))
            elif x > y then
                (=<<) delRight (apply (fun r -> Node(color, left, y, r)) (del right))
            else
                delRoot t
    ...
    and delRoot t =
        ...
        | Node(color, left, _, right) ->
            ...
            (=<<) delRight (apply (fun r -> Node(color, left, (m.Value).Value, r)) (delMin right m))
    ...
    and delMin t m =
        ...
        | Node(color, left, y, right) -> (=<<) delLeft (apply (fun l -> Node(color, l, y, right)) (delMin left m))
